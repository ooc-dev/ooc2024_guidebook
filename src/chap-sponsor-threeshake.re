...
今回の記事については TypeScript で型プログラミングについてです。そもそも型の重要性については、いたるところで言われておりますし、文字数の限りもあるので、今回は「重要である」という前提で、
その型をどうやってプログラミングしていくかにフォーカスしてお話します。

型プログラミングを身につけて、プログラミングにおける Toil をなくしていきましょう。

== TypeScript で型プログラミング

型が重要だからこそ、きちんと書いていく必要があります。とはいえ、複数回同じようなことを書くのは、やはり苦痛です。例えば、型をプログラミングするモチベーションとして、以下のようなパターンがあります。
...
アプリケーションを書いていると関係性を持つ型が存在することがあります。例えば以下のようなパターンです。
...
こちらの場合は明示的に @<code>{Person} のものであることを認識できます。

=== master データとして、配列を用意してそれを元に入力値を制限したい

@<code>{select} などで @<code>{input} のパターンを絞っている場合、コード上にもそれを反映したい場合などがあります。コードでは以下のようなパターンです。
...
そもそも、この @<code>{as const} の値自体を縛りたいという要望も出てきます。拡張もある程度のパターンを持たせたいということです。
その際には @<code>{satisfies} というキーワードを利用します。今回説明はできませんが、頭の隅に置いていただけると幸いです。

=== object A の key と object B の key に一定の関係性がある

例えば、 API から帰ってくる Json の key が snake_case の場合、 TypeScript で camelCase に変換しつつマッピングをします。
...
この例のように API で指針が決まっている場合、変換用の型を書いていくのはとても面倒です。この課題について OpenAPI など自動生成を使うことで対応している方も多いと思います。ここではあえて、自作するとした場合にどうするかを考えます。
...
これには　TypeScript の機能の @<code>{Conditional Types}、@<code>{Template Literal Types} と @<code>{Mapped Types} を利用します。上記を、もう少し詳しく見てみます。
...
残念ながら、私には型プログラミングは複雑に見えがちだと思います。少なくとも私がこれを見て理解するまで数年単位でかかりました。
なので、難しいものは使わない、本当に困っているときに使うとして、そもそも型が簡潔になるような、データ設計、ロジック設計を行うのが良い手段になります。

== まとめ

サンプルを交えて、型のプログラミングをどうやれば良いのかについて、簡単にではありますが説明をさせていただきました。
型を定義するのは重要ですが、面倒な側面もあります。なので楽をしましょう。とはいえ、あまり型を複雑にしないようにしましょう。
私は、どうすればいいかなと悩んだ場合、普通のプログラミングと同じように YAGNI や DRY などの原則を意識してみると良いです。

以上、簡単な型プログラミングについての紹介ではありますが、弊社株式会社スリーシェイク https://3-shake.com/ に興味ありましたら、ぜひお声掛けください。
