...
そもそも型の重要性については、いたるところで言われておりますし、文字数の限りもあるので、今回は「重要である」という前提で、
...
アプリケーションを書いていると関係性を持つ型が存在することがあります。
例えば以下のようなパターンです。
...
@<code>{select} などで @<code>{input} のパターンを絞っている場合、コード上にもそれを反映したい場合などがあります。
コードでは以下のようなパターンです。
...
そもそものこの @<code>{as const} の値自体を縛りたいという要望も出てきます。拡張もある程度のパターンを持たせたいということです。
