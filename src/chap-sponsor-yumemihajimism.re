...
@<code>{UI = f(state)}や @<code>{UI = f(data) }などで示される通り、多くのフロントエンドエンジニアは「UI とは関数である」というメンタルモデルを共有しています。
...
素朴な例を挙げるとすれば、下記のような Greet 関数コンポーネントはいかがでしょう。name を引数に取って、p タグに囲まれた”Hello, 〇〇”という文字列を返します。name に hajime を入れると、 @<code>{<p>Hello, hajime</p>} が返ってくることになります。
...
この勉強会のゴールの 1 つは、OOUI を「オブジェクトを受け取って UI を返す関数」として理解すること、すなわち@<code>{OOUI = f(object) }というメンタルモデルを獲得することです。そしてそのために、オブジェクトを引数に取る関数コンポーネントを記述することに挑戦します。
...
@<code>{OOUI = f(model) }をプログラムで表現していく上で、まず重要なのは「それがどのようなモデルなのか」ということです。
...
下の@<code>{const ProfileCard} の方は、 @<code>{: FC<Props>} の部分を除けば Greet 関数と構造が同じです。 @<code>{interface Props} の方は、@<code>{type Todo} の書き方と大体同じです。
...
要するにモデルの配列を引数に取るということですが（引数名も複数形にしました）、こう見ると@<code>{interface Props}にコンポーネント（関数）の関心が現れることがはっきりわかります。
...
さて、@<code>{?}に入るのはどのようなデータでしょうか。その説明が先程記述した
...
この Profile 型のデータの一例として、「モデルを TypeScript の型で記述する」の章で書いた@<code>{profileA}が挙げられます。
...
//noindent
ということで、この@<code>{profileA}を@<code>{ProfileCard}に渡してあげれば、そのデータに沿った UI が表示されることになります。
...
//embed{
    \begin{minipage}{.1\linewidth}
        \centering
        \includegraphics[width=.75\linewidth]{images/contributors/yumemi-hajimism.png}
    \end{minipage}
    \begin{minipage}{.89\linewidth}
        中川孟　@yumemi\_hajimism https://twitter.com/yumemi\_hajimism\\
    \end{minipage}
    \hspace{1ex}
//}
