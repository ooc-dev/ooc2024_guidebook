...
たしかに、今日のJavaの世界では標準ライブラリにIteratorインターフェースが定義されているほか、拡張for文などで言語仕様レベルでIteratorの呼び出し部分が組み込まれているため、巷でみられるIteratorパターンのサンプルコード全体を自分で実装することはまずないです。一方で、Iteratorパターンに理解があれば、拡張for文に独自のクラスを渡したり、Stream#iterateメソッドを使うときにすぐにイメージが湧いたり...といった恩恵を受けれることでしょう。
