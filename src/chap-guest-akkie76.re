...
//noindent
@<b>{アプリケーションとして一貫性のあるアーキテクチャ、設計となっているか。また、アプリケーションにとって適切な責務・振舞いになっているか。}
...
以下のコードでは、@<b>{add()} で2つのバリデーションと2つのビジネスロジックが実装されており、単純に「商品を追加する」以外の責務を1つの関数の責務として行われています。この関数自体複数の責務を持ってしまっている(責務超過)ため、Design 指摘の対象となります。
...
//noindent
@<b>{可読性のあるコーディングになっているか。処理ができるだけシンプルな振る舞いになっているか。}
...
//noindent
@<b>{変数やクラス、メソッドに責務を意図した明確な名前が付けられているか。英語文法に誤りがないか。typoもこれに含まれる。}
...
//noindent
@<b>{システムとして外部仕様を充足しているか。作者が意図した通りの振る舞いであるか。また、システムの通信量、パフォーマンスに懸念がないか。}
...
//noindent
@<b>{システムとして適切な自動テストを兼ね備えているか。自動テストの内容で品質を担保できているか。また、システムを担保するパラメータ群を備えているか。}
...
//noindent
@<b>{ソースコード上に記載されているdoc、コメントが適切な内容であるか。また、関連するドキュメントは更新されているか。その内容は適切か。}
...
コードレビューでは上記の7つの観点に加えて、指摘修正の要否を4つの類型に分けてコメントをします。
...
実際にコードレビューをするとき、上記の 7 つの観点と修正の温度感をこのように交えた@<b>{Prefix}を付けて、コメントをします。

　

@<b>{MUST(Design)}:ドメインロジックがControllerクラスに実装されてます。 domain層の対象packageに新しくクラスを作成して実装を移してください。
