...
テストコードが期待通りになっていない場合や、テストでのパラメータに考慮漏れがある場合などテストに関連する事項が指摘の対象になります。
...
コードレビューでマージするために必要な修正は MUST 指摘となります。 MUST と SHOULD の使い分けは難しい部分もありますが、SHOULD での指摘事項は別途リリースまでに対応するなど、レビューアーと相談して対応を進めます。一方、IMO や NITS は修正は不要ですが、修正しない場合はその旨をコメントに返信してもらい、コメントを閉じてからマージする運用をしています。具体的な運用方法はチームによって異なるので、チームにあった方法で運用するのが良いと思います。
